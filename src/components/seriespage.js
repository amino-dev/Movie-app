...
function SeriesPage({search,movie,getFavorites,addFavoriteMovies}) {
  return(
  <div className="movies-card container">
   {Object.keys(movie).filter((id) => {
     if ((movie[id].Type === "serie" && search === "") || (movie[id].Type === "serie" && movie[id].Title.toLowerCase().includes(search.toLowerCase()))){
      return id
    }
    }).map(id=>
